...
> **On references, and resisting paper-collecting:** [Latte](https://github.com/Vchitect/Latte) (Jan 2024) enumerates four ways to decompose space and time in a video DiT — interleaved spatial/temporal blocks, factorized spatial-then-temporal, spatial attention with 3D convs, and variants. **It is not state of the art and you do not need to read it properly.** Its one transferable takeaway — *factorization is a small design space, so choose a variant rather than defaulting into one* — is the table above. **Skim it if you want to see the four variants side by side; otherwise skip.** For a current reference *implementation* of DiT inflation, Open-Sora is more used and more maintained.
...
| **9a** Video models | ⭐ **AnimateDiff** (Guo et al.) — the inflation recipe, canonical and with code. ⭐ **Stable Video Diffusion** (Blattmann et al.) — read it **for its data-curation section**, which is the best public account of how video training data actually gets built | ✅ SwiftTry (the two-stage recipe you're implementing); **Wan 2.1/2.2** tech reports — reference material for the backbone Parts G–J build on, not a sit-down read. *Skim only:* Latte (arXiv 2401.03048 — enumerates four space/time decompositions for a video DiT; the takeaway is already in 9a's table); Lumiere (closed; one-paragraph argument that generating all frames at multiple space-time scales beats keyframes-then-interpolate) |
